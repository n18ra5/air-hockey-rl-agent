# -*- coding: utf-8 -*-
"""
BALANCED AIR-HOCKEY - 100% CHASER + DEFENSIVE REPOSITIONING
================================================

BLUE:
    Brand-new PPO agent trained from scratch.

RED:
    100%-speed chaser opponent.

REWARDS
=======
Blue scores:                         +2.0   FIXED
Blue concedes:                       -2.0   FIXED
Blue makes a real puck hit:          Optuna tuned
Gets closer to puck on BLUE half:    Optuna tuned
Does NOT get closer on BLUE half:    Optuna tuned
Slow movement:                       Optuna tuned
Moves toward defensive home when
puck is on RED half:                 +0.002 FIXED

NEW CHANGES IN THIS VERSION
===========================
1. Blue concede penalty is now -2.0.
2. When the puck is on RED's half, BLUE gets +0.002 for each step that
   genuinely reduces its distance to a defensive home position in front
   of BLUE's goal.
3. RED chaser now moves at 100% paddle speed.

The previously discussed extra +0.01 hit bonus is NOT included.
The existing Optuna-tuned real-hit reward is left unchanged.
"""
...
# ================================================================
# DEFENSIVE REPOSITIONING REWARD
# ================================================================
# Fixed new reward requested for this experiment.
#
# It is only available while the puck is on RED's half.
# BLUE receives the reward only when its movement genuinely reduces
# its distance to the defensive home position, so simply sitting there
# does not farm reward.

DEFENSIVE_REPOSITION_REWARD = 0.002

# Defensive home position:
# centre of the table, in front of BLUE's bottom goal.
DEFENSIVE_HOME_X_FRACTION = 0.50
DEFENSIVE_HOME_Y_FRACTION = 0.85
...
if QUICK_TEST:
    OUTPUT_FOLDER = "balanced_ai_full_chaser_test"
else:
    OUTPUT_FOLDER = "balanced_ai_full_chaser"
...
OPTUNA_DB_PATH = os.path.join(
    OUTPUT_FOLDER,
    "balanced_full_chaser_optuna.db"
)

OPTUNA_STORAGE = "sqlite:///" + os.path.abspath(
    OPTUNA_DB_PATH
)
...
class BalancedTrainingEnv(AirHockeyEnv):
    ...
    def step(self, blue_action):
        ...
        # Save BLUE's position before movement so we can determine whether
        # this step moved it closer to the defensive home position.
        old_blue_position = self.paddle1_pos.copy()
        ...
        # Defensive repositioning reward
        # --------------------------------------------------------
        # Only applies when the puck is on RED's half.
        #
        # Reward BLUE for moving closer to a home position directly
        # in front of its own goal. No reward is given merely for
        # already being there.

        puck_on_red_half = (
            self.puck_pos[1] < self.height / 2
        )

        if puck_on_red_half:

            defensive_home = np.array(
                [
                    self.width * DEFENSIVE_HOME_X_FRACTION,
                    self.height * DEFENSIVE_HOME_Y_FRACTION
                ],
                dtype=np.float32
            )

            old_distance_to_home = float(
                np.linalg.norm(
                    old_blue_position - defensive_home
                )
            )

            new_distance_to_home = float(
                np.linalg.norm(
                    self.paddle1_pos - defensive_home
                )
            )

            if new_distance_to_home < old_distance_to_home:
                reward += DEFENSIVE_REPOSITION_REWARD
        ...
        if player1_hit:
            # Existing Optuna-tuned hit reward only.
        ...
        # Re-check walls after paddle collisions. A paddle collision can
        # otherwise displace the puck partly into a wall on the same step.
        self._handle_wall_collisions()

        # --------------------------------------------------------
        # PUCK-STALL DETECTION
        # --------------------------------------------------------
...
def objective(trial):
    ...
    for seed in TRAINING_SEEDS:
        ...
        # Fresh Stage 1 training:
        # create a brand-new PPO agent for every trial/seed.
        model = PPO(
            "MlpPolicy",
            env,
            seed=seed,
        ...
        row = {
            ...
            "no_approach_penalty": no_approach_penalty,
            "slow_movement_penalty": slow_movement_penalty,
            "defensive_reposition_reward": DEFENSIVE_REPOSITION_REWARD,
            "defensive_home_x_fraction": DEFENSIVE_HOME_X_FRACTION,
            "defensive_home_y_fraction": DEFENSIVE_HOME_Y_FRACTION,
            "chaser_speed_fraction": CHASER_SPEED_FRACTION,
...
def run_optuna():
    ...
    study = optuna.create_study(
        study_name="balanced_stage1_full_chaser_new_rewards",
...
#%%
# ================================================================
# CELL 1 - RUN FRESH STAGE 1 TRAINING
# ================================================================
...
#%%
# ================================================================
# CELL 2 - WATCH A TRAINED STAGE 1 MODEL
# ================================================================
